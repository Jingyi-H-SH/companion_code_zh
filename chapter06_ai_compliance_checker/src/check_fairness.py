...
REFERENCES = [item for item in load_json_items(REFERENCE_PATH) if item["section"] == "fairness"]
SCHEMA = {
    "type": "object",
    "properties": {
...
def run_fairness_check(system_profile: dict, checklist_items: list[dict]) -> dict:
    ...
    result = json_completion(
        "You review fairness and governance practices for health AI systems. Use the retrieved governance references as grounding context and return concise evidence-based JSON.",
        prompt,
        "fairness_review",
        SCHEMA,
    )
