...
REFERENCES = [item for item in load_json_items(REFERENCE_PATH) if item["section"] == "privacy"]
SCHEMA = {
    "type": "object",
    "properties": {
...
def run_privacy_check(system_profile: dict, checklist_items: list[dict]) -> dict:
    ...
    result = json_completion(
        "You review privacy safeguards for health AI systems. Use the retrieved governance references as grounding context and return concise evidence-based JSON.",
        prompt,
        "privacy_review",
        SCHEMA,
    )
